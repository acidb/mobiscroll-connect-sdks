...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function filterCalendars(prov, selectedCalendarId) {
    const prev = calendarSel.value;
    if (!prov) {
      calendarSel.innerHTML = '<option value="">Select a provider first…</option>';
      return;
    }
    calendarSel.innerHTML = '<option value="">Select a calendar…</option>';
    (window.CAL_DATA || [])
      .filter(function (c) { return c.provider === prov; })
      .forEach(function (c) {
        const o = document.createElement('option');
        o.value = c.id;
        o.textContent = c.title;
        if (c.id === (selectedCalendarId || prev)) o.selected = true;
        calendarSel.appendChild(o);
      });
  }

  providerSel.addEventListener('change', function () {
    filterCalendars(this.value, null);
  });

  // Apply filter on load (handles URL params like ?provider=google&calendarId=...).
  filterCalendars(providerSel.value, calendarSel.value || null);
